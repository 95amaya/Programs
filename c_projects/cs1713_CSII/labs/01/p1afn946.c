/**********************************************************************
p1afn946.c  (change the name to use your abc123 id)
Purpose:
...
Notes:
    * Method exits when error is found with input file *
...
void processCustomers()
{
    ...
    while(fgets(szInputBuffer, MAX_LINE_SIZE, pFileCust) != NULL)
    {
        ...
        if(fgets(szInputBuffer, MAX_LINE_SIZE, pFileCust) != NULL)
        {
            iScanfCnt = 0;
            ...
            printf("%s\n%s %s %s\n", customer.szStreetAddress, customer.szCity, customer.szStateCd, customer.szZipCd); 
        }
        
        // Reading in book transactions
        ...
        while(fgets(szInputBuffer, MAX_LINE_SIZE, pFileCust) != NULL)
        {
            ...
            printf("%3c%13s%12s\n", trans.cTransType, trans.szBookId, trans.szTransDt);
