...
Input:
    Book	same as Programming Assignment #4 with binary trees.
...
void prettyPrintT(NodeT *p, int iIndent)
{
    ...
    // output each book in tree format
    for (i = 0; i < iIndent; i++)
    {
        printf("    "); // binary tree spacing for each node level
    }
    ...
    else if (strcmp(pszSubCommand, "TRANS") == 0)
    {
 	// reset scanf counter
	    ...
	    // print formatted trasaction values from transaction structure
            ...
            pCustomer->dFeeBalance += processTransaction(pRoot, *pCustomer, transaction); // Process customer trasaction
...
void processBookCommand(NodeT **ppRoot, char *pszSubCommand, char *pszRemainingInput)
{
    ...
    if(strcmp(pszSubCommand, "PPRINT") == 0)
    {
        prettyPrintT(*ppRoot, 1);
    }
    else if(strcmp(pszSubCommand, "NEW") == 0)
    {
        ...
        pCheck = searchT(*ppRoot, book.szBookId);
    ...
    else if(strcmp(pszSubCommand, "CHANGE") == 0)
    {
        ...
        iKey = searchT(*ppRoot, book.szBookId);
...
NodeT * searchT(NodeT *p, char szMatchBookId[])
{
    ...
    /*  Starting with the Root of LL and traversing through each node by getting address 
...
NodeT * insertT(NodeT *p, Book book)
{
    ...
    if(strcmp(p->book.szBookId, book.szBookId) == 0)
        return p;
    else if(strcmp(p->book.szBookId, book.szBookId) < 0)
...
double processTransaction(NodeT *pRoot, Customer customer, Transaction transaction)
{
    ...
    NodeT *iKey = NULL;  // used to find the book Node in Book Linked List
    switch(transaction.cTransType)
    {
        /***********************_CheckOut case_*******************************/
        case 'C': case 'c':
            iKey = searchT(pRoot, transaction.szBookId); // Find book in the library
